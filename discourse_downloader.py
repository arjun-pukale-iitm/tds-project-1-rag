...
def extract_thread_data(file_path):
  with open(file_path, 'r', encoding='utf-8') as f:
    ...
    if(len(posts) > 1): #someone has answered the question
      ...
      for post in posts[1:]:
        ...
        post_body = post['cooked']
        post_url = post['post_url']
        replied_to_post_number = post['reply_to_post_number']
        ...
        post_replied_to = None
        if(replied_to_post_number):
          for reply_post in posts:
            if(reply_post['post_number'] == replied_to_post_number):
              post_replied_to = reply_post['cooked']
              break
        post_data['post_replied_to'] = post_replied_to

        # print(post_data)
        # print("\n
        all_thread_posts.append(post_data)

if __name__ == "__main__":
    download()
    print("\n================================")
    print("Starting preprocessing of downloaded discourse contents")
    # call extract_thread_data for all files in discourse post folder
    for filename in os.listdir(OUTPUT_DIR):
        if filename.endswith('.json'):
            file_path = os.path.join(OUTPUT_DIR, filename)
            extract_thread_data(file_path)
    with open('discourse_content.json', 'w') as f:
        json.dump(all_thread_posts, f, indent=2)
        print("\ndiscourse content saved to discourse_content.json")
        print("Discourse data preprocessing complete.")